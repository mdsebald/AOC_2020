defmodule Day19 do
  ...
  the connection to the satellite is having problems, and many of the messages sent back from the satellite have been corrupted.
  ...
  def msgs_match_rule0_1 do
    # "inputs/day19_test1_input.txt")
    {rules, msgs} = get_rules_msgs()

    regex_bin =
      build_regex_match(rules)
      |> (fn str -> "^#{str}$" end).()
      |> Regex.compile!()

    # IO.puts("#{inspect(regex_bin)}")
    Enum.count(msgs, &(&1 =~ regex_bin))
  end
  ...
  def msgs_match_rule0_2 do
    # "inputs/day19_test2_input.txt")
    {rules, msgs} = get_rules_msgs()
    # modify these rules
    # 8: 42 | 42 8
    # 11: 42 31 | 42 11 31
    rules = mod_rule(rules, "8", ["42", "|", "42", "8"])
    rules = mod_rule(rules, "11", ["42", "31", "|", "42", "11", "31"])
  ...
  defp mod_rule(rules, rule_num, rule) do
    Map.put(rules, rule_num, rule)
  end

  # Common functions

  defp get_rules_msgs(input \\ "inputs/day19_input.txt") do
    [rule_strs, msg_strs] = File.read!(input) |> String.split("\n\n")
    {build_rules_map(rule_strs), String.split(msg_strs, "\n")}
  end

  defp build_rules_map(rule_strs) do
    String.split(rule_strs, "\n")
    |> Enum.reduce(%{}, fn rule_str, rules_map ->
      [key | rules] = String.split(rule_str)
      ...
      Map.put(rules_map, key, rules)
    end)
  end

  defp build_regex_match(rules), do: build_regex_match(rules, "0", "", 6)

  defp build_regex_match(_rules, _rule_num, regex_str, 0), do: regex_str

  defp build_regex_match(_rules, "a", regex_str, _loop_limit), do: regex_str <> "a"

  defp build_regex_match(_rules, "b", regex_str, _loop_limit), do: regex_str <> "b"

  defp build_regex_match(_rules, "|", regex_str, _loop_limit), do: regex_str <> "|"

  defp build_regex_match(_rules, "(", regex_str, _loop_limit), do: regex_str <> "("

  defp build_regex_match(_rules, ")", regex_str, _loop_limit), do: regex_str <> ")"

  defp build_regex_match(rules, rule_num, regex_str, loop_limit) do
    # a rule is:
    #   "a" (occurs only once) or
    #   "b" (occurs only once) or
    #   1 or more rule numbers or
    #   1 or more rule numbers "|" 1 or more rule numbers

    rule = rules[rule_num]
    # If a rule contains a loop, limit the number of times we can loop on it.
    # Just need to loop enough cycles to match all possible messages in our input.
    # Via trial and error, we determined 6 is enough loops.
    loop_limit =
      case Enum.member?(rule, rule_num) do
        true -> loop_limit - 1
        false -> 6
      end

    # IO.puts("#{rule_num}, #{inspect(rule)}, #{inspect(regex_str)}")
    if Enum.member?(rule, "|") do
      regex_str = build_regex_match(rule, "(", regex_str, loop_limit)
      regex_str = Enum.reduce(rule, regex_str, &build_regex_match(rules, &1, &2, loop_limit))
      build_regex_match(rule, ")", regex_str, loop_limit)
    else
      Enum.reduce(rule, regex_str, &build_regex_match(rules, &1, &2, loop_limit))
    end
  end
end
