defmodule Day13 do
  ...
  After that, each bus travels to the airport, then various other locations, and
  ...
  """
  def get_earliest_bus do
    {bus_num, wait_time} = get_bus_schedule() |> find_earliest_bus
    bus_num * wait_time
  end

  defp find_earliest_bus({earliest_depart, bus_sched}) do
    bus_sched
    |> Enum.reduce({0, 9_999_999}, fn bus_num, {earliest_bus_num, min_wait_time} ->
      if is_integer(bus_num) do
        wait_time = bus_num * (div(earliest_depart, bus_num) + 1) - earliest_depart
        # IO.puts("#{bus_num}, #{wait_time}")
        if wait_time < min_wait_time do
          {bus_num, wait_time}
        else
          {earliest_bus_num, min_wait_time}
        end
      else
        {earliest_bus_num, min_wait_time}
      end
  ...
  # Had a brute force solution that did not look like it would finish in 24 hours
  # "Borrowed" LostKobrakai's solution: https://github.com/LostKobrakai/aoc2020/blob/master/lib/aoc2020/day13.ex

  def get_earliest_timestamp do
    {_earliest_depart, bus_sched} = get_bus_schedule()

    get_buses_and_offsets(bus_sched)
    |> Enum.reduce({0, 1}, fn {bus, index}, {t, step} ->
      t =
        Stream.unfold(t, fn t ->
          # IO.puts("Unfold: #{t}, #{t+step}")
          {t, t + step}
        end)
        |> Stream.filter(fn t ->
          # IO.puts("Testing: #{t + index}, #{bus}")
          rem(t + index, bus) == 0
        end)
        |> Enum.at(0)
  ...
  defp lcm(a, b) do
    lcm = div(a * b, Integer.gcd(a, b))
    # IO.puts("LCM: #{a}, #{b} = #{lcm}")
    lcm
  end

  defp get_buses_and_offsets(bus_sched) do
    Enum.with_index(bus_sched)
    |> Enum.reject(fn {bus, _offset} -> bus == "x" end)
  end

  # Common functions

  defp get_bus_schedule(input \\ "inputs/day13_input.txt") do
    [earliest_depart_str, bus_schedule_str] =
      File.read!(input)
      |> String.split()
    ...
    bus_schedule =
      bus_schedule_str
      |> String.split(",")
      |> Enum.map(&get_bus_number/1)

    {String.to_integer(earliest_depart_str), bus_schedule}
  end

  defp get_bus_number("x"), do: "x"

  defp get_bus_number(number_str), do: String.to_integer(number_str)
end
