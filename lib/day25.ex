defmodule Day25 do
  ...
  and the elevators are also temporarily out of service,
  ...
  def find_encryption_key do
    # "inputs/day25_test1_input.txt")
    [card_public_key, door_public_key] = get_public_keys()

    # card_loop_count = find_loop_count(card_public_key, 1, 0)
    door_loop_count = find_loop_count(door_public_key, 1, 0)

    # IO.puts("loop count: card: #{card_loop_count}, door: #{door_loop_count}")

    _card_encryption_key = find_encryption_key(1, card_public_key, door_loop_count)
    # door_encryption_key = find_encryption_key(1, door_public_key, card_loop_count)

    # card and door encryption keys are identical
    # IO.puts("encryption_key: card: #{card_encryption_key}, door: #{door_encryption_key}")
  end

  defp find_loop_count(public_key, value, count) do
    if public_key == value do
      count
    else
      find_loop_count(public_key, transform(value, 7), count + 1)
    end
  end

  defp find_encryption_key(encryption_key, public_key, count) do
    if count == 0 do
      encryption_key
    else
      find_encryption_key(transform(encryption_key, public_key), public_key, count - 1)
    end
  end
  ...
  and you're the only one that knows how to fix it! We've already dispatched a reindeer
  ...
  defp get_public_keys(input \\ "inputs/day25_input.txt") do
    File.read!(input)
    |> String.split("\n", trim: true)
