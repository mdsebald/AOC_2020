defmodule Day22 do
  ...
  def winning_score_1 do
    # "inputs/day22_test1_input.txt")
    [p1, p2] = get_cards_dealt()
    # IO.puts("P1: #{inspect(p1)}, P2: #{inspect(p2)}")
    play(p1, p2) |> calc_score()
  end
  ...
  defp play([c1 | rem_p1], [c2 | rem_p2]) do
    if c1 > c2 do
      play(rem_p1 ++ [c1, c2], rem_p2)
    else
      play(rem_p1, rem_p2 ++ [c2, c1])
    end
  end
  ...
  as the number on the card they just dealt, the winner of the round is determined by recursing into a sub-game of Recursive Combat.
  ...
  # Borrowed heavily from thth https://github.com/thth/aoc_2020/blob/master/22.exs

  def winning_score_2 do
    # "inputs/day22_test1_input.txt")
    [p1, p2] = get_cards_dealt()
    # IO.puts("P1: #{inspect(p1)}, P2: #{inspect(p2)}")
    {_player, hand} = new_game(p1, p2)
    calc_score(hand)
  end

  defp new_game(p1, p2) do
    play_r(p1, p2, [])
  end

  defp play_r(p1, p2, prev_hands) do
    cond do
      Enum.member?(prev_hands, {p1, p2}) ->
        # IO.puts("Next hand matches a previous hand")
        {:p1, p1}
      ...
      length(tl(p1)) >= hd(p1) and length(tl(p2)) >= hd(p2) ->
        [c1 | rem_p1] = p1
        [c2 | rem_p2] = p2
        r_p1 = Enum.take(rem_p1, c1)
        r_p2 = Enum.take(rem_p2, c2)

        case new_game(r_p1, r_p2) do
          {:p1, _} -> play_r(rem_p1 ++ [c1, c2], rem_p2, [{p1, p2} | prev_hands])
          {:p2, _} -> play_r(rem_p1, rem_p2 ++ [c2, c1], [{p1, p2} | prev_hands])
        end

      hd(p1) > hd(p2) ->
        [c1 | rem_p1] = p1
        [c2 | rem_p2] = p2
        play_r(rem_p1 ++ [c1, c2], rem_p2, [{p1, p2} | prev_hands])

      hd(p2) > hd(p1) ->
        [c1 | rem_p1] = p1
        [c2 | rem_p2] = p2
        play_r(rem_p1, rem_p2 ++ [c2, c1], [{p1, p2} | prev_hands])
    end
  end

  # Common functions

  defp get_cards_dealt(input \\ "inputs/day22_input.txt") do
    File.read!(input)
    |> String.split("\n\n", trim: true)
    |> Enum.map(&String.split(&1, "\n", trim: true))
    |> Enum.map(fn player_list ->
      Enum.reject(player_list, &String.starts_with?(&1, "Player"))
      |> Enum.map(&String.to_integer/1)
    end)
  end

  defp calc_score(hand) do
    # IO.puts("#{inspect(hand)}")
    Enum.reverse(hand)
    |> Enum.with_index(1)
    |> Enum.reduce(0, fn {card, index}, score ->
      score + card * index
    end)
  end
end
