defmodule Day01 do
  ...
  """
  def fix_expense_report do
    exps_list = File.read!("day01_input.txt")
    |> String.split
    |> Enum.map(&String.to_integer/1)
    # Don't think AOC committee would give us bad data, but doesn't hurt to check
    |> Enum.filter(fn exp -> ((0 <= exp) && (exp <= 2020)) end)

    IO.puts("2020 Expense Pair: #{find_2020_exp_pair(exps_list)}\n")
    IO.puts("2020 Expense Triplet: #{find_2020_exp_triplet(exps_list)}\n")
  end
